...
print('Program To Implement a Decision Tree -- Regression ProblemT')
...
# Identify the target column (Salary) and feature columns
X = df.drop(columns=['Salary'])  # Features are all columns except Salary
print('Feature values:  ',X.columns)

#since salary column is the target/ output variable let us keep all columns n drop only salary

#  create an empty column named salary for output variable
...
clf = DecisionTreeRegressor()    #Salary is a continuous value hence we use regression
clf.fit(X_train, Y_train) #Training Decision Tree Classifier


y_test_pred = clf.predict(X_test)
print('Predicted Salaries of Testing Samples:')
print(y_test_pred)
